<div class="app-sidebar sidebar-shadow">
                    <div class="app-header__logo">
                        <div class="logo-src"></div>
                        <div class="header__pane ml-auto">
                            <div>
                                <button type="button" class="hamburger close-sidebar-btn hamburger--elastic" data-class="closed-sidebar">
                                    <span class="hamburger-box">
                                        <span class="hamburger-inner"></span>
                                    </span>
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class="app-header__mobile-menu">
                        <div>
                            <button type="button" class="hamburger hamburger--elastic mobile-toggle-nav">
                                <span class="hamburger-box">
                                    <span class="hamburger-inner"></span>
                                </span>
                            </button>
                        </div>
                    </div>
                    <div class="app-header__menu">
                        <span>
                            <button type="button" class="btn-icon btn-icon-only btn btn-primary btn-sm mobile-toggle-header-nav">
                                <span class="btn-icon-wrapper">
                                    <i class="fa fa-ellipsis-v fa-w-6"></i>
                                </span>
                            </button>
                        </span>
                    </div>    <div class="scrollbar-sidebar">
                        <div class="app-sidebar__inner">
                            <ul class="vertical-nav-menu">
                                <li class="app-sidebar__heading">Device Manager</li>
                                <li>
                                    <a href="add_device.php">
                                        <i class="metismenu-icon pe-7s-display2"></i>
                                        Register New Device
                                    </a>
                                </li>

                                <li>
                                    <a href="view_devices.php">
                                        <i class="metismenu-icon pe-7s-display2"></i>
                                        View Registered Devices
                                    </a>
                                </li>

                                <li class="app-sidebar__heading">Student's Lending System</li>
                                <li>
                                    <a href="borrow_system.php">
                                        <i class="metismenu-icon pe-7s-display2"></i>
                                        Borrowing System
                                    </a>
                                </li>

                                <li>
                                    <a href="return_system.php">
                                        <i class="metismenu-icon pe-7s-display2"></i>
                                        Return System
                                    </a>
                                </li>

                                <li>
                                    <a href="lending_view.php">
                                        <i class="metismenu-icon pe-7s-display2"></i>
                                        View
                                    </a>
                                </li>

                                <li class="app-sidebar__heading">Files</li>
                                <li>
                                    <a href="upload.php">
                                        <i class="metismenu-icon pe-7s-file"></i>
                                        File Upload
                                    </a>
                                </li>
                                <li>
                                    <a href="uploads.php">
                                        <i class="metismenu-icon pe-7s-cloud-upload"></i>
                                        View Uploaded Files
                                    </a>
                                </li>


                                <li class="app-sidebar__heading">Profile</li>
                                <li>
                                    <a href="profile.php">
                                        <i class="metismenu-icon pe-7s-user">
                                        </i>View Profile
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                </div>